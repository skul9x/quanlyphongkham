...
class EditDiagnosisWindow(QDialog):
    def __init__(self, parent=None, visit_id=None, on_success_callback=None):
        ...
        self.setWindowTitle(f"Sửa Chẩn Đoán & Thuốc - {name}")
        self.setModal(True)
    ...
    def setup_ui(self):
        ...
        title = QLabel("Chỉnh Sửa Lâm Sàng")
        ...
        layout.addWidget(self.lbl("Danh sách thuốc (Text):"))
        self.medicine_edit = QTextEdit()
        self.medicine_edit.setPlaceholderText("1) Thuốc A...\n2) Thuốc B...")
        # Updated style
        self.medicine_edit.setStyleSheet("""
            QTextEdit { 
        ...
        """)
        layout.addWidget(self.medicine_edit)
        ...
        if self.visit_data:
            ...
            if lines:
                ...
                self.medicine_edit.setPlainText('\n'.join(lines[1:]))
    ...
    def save(self):
        ...
        m = self.medicine_edit.toPlainText().strip()
        ...
        if database.update_visit_details_db(self.visit_id, w, full):
