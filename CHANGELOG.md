...
## [5.0.5] - 2026-02-23
### Added
- Tính năng: Tùy chỉnh "Tiền công khám" (Consultation Fee) trong phần Cài đặt.
- Script đóng gói chuyên nghiệp: `build_deb.sh` để tạo bộ cài `.deb` cho Linux.
- Script tạo phím tắt: `install_shortcut.sh`.
- File hướng dẫn: `donggoi_linux.txt`.

### Fixed
- Lỗi `AttributeError`: Xử lý đúng kiểu dữ liệu `sqlite3.Row` khi lưu đơn thuốc trong `ui_prescription_window_pyside.py`.
- Lỗi Đồng bộ Cloud (23503): Đảm bảo danh mục thuốc (Medicines) được đẩy lên Cloud trước Bệnh nhân và Đơn thuốc trong `sync_manager.py`.
- Cải thiện UI: Tổng cộng đơn thuốc tự động tính theo Tiền khám tùy chỉnh.
